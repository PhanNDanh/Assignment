...
@Repository
public interface GuestProductDetailRepository extends ProductDetailRepository {

    @Query(nativeQuery = true, value = """
            SELECT product_detail.id, product.name AS productName, category.name AS categoryName, color.name AS colorName, size.name AS sizeName, material.name AS materialName, price, description
                        FROM product_detail
   ...
   Page<GuestProductDetailResponse> getAllProductDetailResponse(Pageable pageable, @Param("productName") String productName,
   ...
   @Param("materialName") String materialName, @Param("price1") BigDecimal price1, @Param("price2") BigDecimal price2);
    ...
    GuestProductDetailResponse getProductDetailResponse(@Param("id") String id);
